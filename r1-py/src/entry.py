...
class Default(WorkerEntrypoint):
    ...
    async def fetch(self, request, env):
        ...
        try:
            ...
            # News source admin API (manage KV `providers.news` / device `newsConfig`)
            if path == "/r1/admin/news":
                return await self.handle_news_admin(request, env)
    ...
    # MARK: - News Source Admin Handler

    NEWS_ADMIN_CORS = {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, x-r1-admin-token",
    }

    def _get_admin_token(self, env) -> str:
        try:
            return getattr(env, "ADMIN_TOKEN") or ""
        except Exception:
            return ""

    async def _read_providers(self, env) -> dict:
        providers_str = await env.R1.get("providers")
        if providers_str:
            try:
    ...
    async def handle_news_admin(self, request, env):
        """GET  /r1/admin/news                              -> list providers + device defaults
        POST /r1/admin/news {action, provider, endpoint|chanId, serial?}
             action: add | remove | set_default | clear_default
        All requests require header `x-r1-admin-token` matching the ADMIN_TOKEN secret.
        """
        if request.method.upper() == "OPTIONS":
            return Response.json({}, headers=self.NEWS_ADMIN_CORS)

        token = request.headers.get("x-r1-admin-token", "")
        expected = self._get_admin_token(env)
        if not expected:
            return Response.json(
                {"error": "ADMIN_TOKEN secret not configured. Run: wrangler secret put ADMIN_TOKEN"},
                status=500, headers=self.NEWS_ADMIN_CORS,
            )
        if token != expected:
            return Response.json({"error": "unauthorized"}, status=401, headers=self.NEWS_ADMIN_CORS)

        try:
            providers_config = await self._read_providers(env)
            news_list = providers_config.get("news", [])
            if not isinstance(news_list, list):
                news_list = []

            if request.method.upper() == "GET":
                serial = request.headers.get("x-r1-serial", "DEFAULT")
                device_str = await env.R1.get(f"device:{serial}")
                news_config = {}
                if device_str:
                    try:
                        news_config = json.loads(device_str).get("newsConfig", {})
                    except Exception:
                        news_config = {}
                return Response.json(
                    {"providers": news_list, "newsConfig": news_config},
                    headers=self.NEWS_ADMIN_CORS,
                )
            ...
            if request.method.upper() == "POST":
                ...
                chan_id = str(body.get("chanId") or "").strip()
                serial = (body.get("serial") or "DEFAULT").strip()

                if action == "add":
                    if not provider or not (endpoint or chan_id):
                        return Response.json(
                            {"error": "provider and (endpoint or chanId) are required"},
                            status=400, headers=self.NEWS_ADMIN_CORS,
                        )
                    news_list = [e for e in news_list if not (isinstance(e, dict) and e.get("provider") == provider)]
                    entry = {"provider": provider}
                    if endpoint:
                        entry["endpoint"] = endpoint
                    else:
                        entry["chanId"] = chan_id
                    news_list.append(entry)
                elif action == "remove":
                    if not provider:
                        return Response.json({"error": "provider is required"}, status=400, headers=self.NEWS_ADMIN_CORS)
                    news_list = [e for e in news_list if not (isinstance(e, dict) and e.get("provider") == provider)]
                elif action == "set_default":
                    if not endpoint:
                        return Response.json({"error": "endpoint is required"}, status=400, headers=self.NEWS_ADMIN_CORS)
                    key = f"device:{serial}"
                    device_str = await env.R1.get(key)
                    device_cfg = {}
                    if device_str:
                        try:
                            device_cfg = json.loads(device_str)
                        except Exception:
                            device_cfg = {}
                    device_cfg["newsConfig"] = {"endpoint": endpoint, "provider": provider}
                    await env.R1.put(key, json.dumps(device_cfg))
                    return Response.json({"ok": True, "serial": serial}, headers=self.NEWS_ADMIN_CORS)
                    ...
                    if device_str:
                        try:
                            device_cfg = json.loads(device_str)
                            device_cfg.pop("newsConfig", None)
                            await env.R1.put(key, json.dumps(device_cfg))
                        except Exception:
                            pass
                    return Response.json({"ok": True, "serial": serial}, headers=self.NEWS_ADMIN_CORS)
                    ...
                    return Response.json(
                        {"error": "unknown action, expected add | remove | set_default | clear_default"},
                        status=400, headers=self.NEWS_ADMIN_CORS,
                    )

                providers_config["news"] = news_list
                await env.R1.put("providers", json.dumps(providers_config))
                return Response.json({"ok": True, "providers": news_list}, headers=self.NEWS_ADMIN_CORS)

            return Response.json({"error": "method not allowed"}, status=405, headers=self.NEWS_ADMIN_CORS)
        except Exception as e:
            return Response.json({"error": str(e), "type": type(e).__name__}, status=500, headers=self.NEWS_ADMIN_CORS)
