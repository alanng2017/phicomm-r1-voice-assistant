...
# ─── 新闻源配置 ────────────────────────────────────────────────────────────────
# 内置央广源默认频道(中国之声)。更多频道/自定义源在 KV `providers` 中登记:
#   {"news": [
#     {"provider": "中国之声", "chanId": "64"},
#     {"provider": "经济之声", "chanId": "xx"},
#     {"provider": "澎湃新闻", "endpoint": "https://your-news-service"}
#   ]}
# endpoint 型源需实现媒体搜索协议: GET {endpoint}/search?keyword=新闻
# 返回 {"count": N, "musicinfo": [{"id", "title", "artist", "url"}]},url 须为可直接播放的音频。
CNR_NEWS_URL = "https://apppc.cnr.cn/cnr45609411d2c5a16/e281277129d478c12c2ed58e84ca906b/f76a0411ae1ff31be9f9e28f0b51348b"
CNR_DEFAULT_CHANNEL_ID = "64"  # 中国之声
...
class R1Tools:
    ...
    def _news_pref_from_header(self) -> str:
        """从 x-r1-news 请求头(base64 JSON)解析偏好新闻源名称,与音乐源机制保持一致"""
        header_val = self.request_headers.get("x-r1-news", "")
        if not header_val:
            return ""
        try:
            decoded = base64.b64decode(header_val).decode("utf-8")
            return json.loads(decoded).get("provider", "")
        except Exception as e:
            print(f"Error decoding x-r1-news header: {e}")
            return ""

    def _resolve_news_source(self, source: str = ""):
        """解析新闻源,返回 (endpoint, chan_id, matched_name)。

        优先级:
        1. 语音指定的 source 名称,在 KV providers.news 中匹配
           (endpoint 型为外部媒体服务,chanId 型为内置央广频道)
        2. x-r1-news 请求头中的偏好 provider
        3. 设备 newsConfig 默认 endpoint
        4. 都没有时返回内置央广中国之声 (chanId)
        """
        name = source or self._news_pref_from_header()
        if name and name != "default":
            provider_list = self.providers_config.get("news", [])
            if isinstance(provider_list, list):
                for entry in provider_list:
                    if isinstance(entry, dict) and entry.get("provider") == name:
                        endpoint = entry.get("endpoint", "").rstrip("/")
                        if endpoint:
                            print(f"[news] matched provider '{name}' -> {endpoint}")
                            return endpoint, "", name
                        chan_id = str(entry.get("chanId", "") or "")
                        if chan_id:
                            print(f"[news] matched built-in channel '{name}' chanId={chan_id}")
                            return "", chan_id, name
            print(f"[news] provider '{name}' not in providers list, trying device default")

        default_endpoint = self.device_config.get("newsConfig", {}).get("endpoint", "").rstrip("/")
        if default_endpoint:
            return default_endpoint, "", name
        return "", "", name
    ...
    @tool
    async def playNews(self, user_input: str, source: Optional[str] = "") -> dict:
        """用于播放新闻,支持多种新闻源。
        samples: 播放新闻, 播放中国之声新闻, 播放经济之声新闻

        Args:
            user_input: 用户输入关键词或描述
            source: 新闻源名称,例如 中国之声、经济之声;留空时使用设备默认新闻源
        """
        endpoint, chan_id, matched = self._resolve_news_source(source)

        # ── 1. 自定义媒体端点型新闻源 ──────────────────────────────────────
        if endpoint:
            data, r1_headers = await self._fetch_media(
                "newsConfig", source or user_input or "新闻", "musicinfo", endpoint
            )
            if isinstance(data, dict) and data.get("count"):
                display = f"{matched or source}新闻" if (matched or source) else "新闻"
                return self._build_playback_response(data, display, "cn.yunzhisheng.music", r1_headers)
            print(f"[news] endpoint source '{matched or source}' returned nothing, fallback to built-in CNR")

        # ── 2. 内置央广频道(默认中国之声) ──────────────────────────────────
        if not chan_id:
            chan_id = CNR_DEFAULT_CHANNEL_ID
            channel_name = "中国之声"
        else:
            channel_name = matched or source or "中国之声"
        ...
        data = {"count": 0, "musicinfo": []}
        try:
            print(f"[news] fetching from cnr chanId={chan_id}...")
            resp = await fetch(CNR_NEWS_URL, method="POST", headers=headers, body=json.dumps(body))
            if resp.ok:
        ...
        r1_headers = {"r1-sname": "cn.yunzhisheng.music"}
        display = "新闻" if channel_name == "中国之声" and not source else f"{channel_name}新闻"
        return self._build_playback_response(data, display, "cn.yunzhisheng.music", r1_headers)
